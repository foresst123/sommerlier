    ...
    def process(self, segments: List[EnhancedSegment], audio: AudioData, enable_word_timestamps: bool = False) -> List[TranscriptSegment]:
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=3) as executor:
            for seg in segments:
                ...
                fw = executor.submit(self._run_whisper, audio_16k, dummy_vad)
                fp = executor.submit(self._run_phowhisper, audio_16k)
                fq = executor.submit(self._run_qwen3, audio_16k, seg.index, tmp_dir)
